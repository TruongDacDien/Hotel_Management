        ...
        private void click_Luu(object sender, RoutedEventArgs e)
        {
            try
            {
                var ddxq = new DDXQ
                {
                    ...
                    DanhGia = double.TryParse(txbDanhGia.Text.Trim(), out var dg) ? dg : 0,
                    ViDo = double.TryParse(txbViDo.Text.Trim(), out var vido) ? vido : 0,
                    KinhDo = double.TryParse(txbKinhDo.Text.Trim(), out var kinhdo) ? kinhdo : 0,
                    KhoangCach = double.TryParse(txbKhoangCach.Text.Trim(), out var kc) ? kc : 0,
