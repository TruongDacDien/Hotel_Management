...
namespace GUI.View
{
	...
	public partial class XuatBaoCaoThongKe : Window
	{
		...
		{
			MonthSelector.SelectionChanged -= OnMonthYearChanged; // Gỡ sự kiện
			YearSelector.SelectionChanged -= OnMonthYearChanged;

			MonthSelector.ItemsSource = Enumerable.Range(1, 12).Select(m => $"{m}").ToList();
			YearSelector.Items.Clear();
			for (var year = DateTime.Now.Year - 10; year <= DateTime.Now.Year; year++)
			{
			...
			MonthSelector.SelectionChanged += OnMonthYearChanged; // Gắn lại sự kiện
			YearSelector.SelectionChanged += OnMonthYearChanged;
